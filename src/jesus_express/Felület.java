...
public class Felület implements ActionListener {
	...
	static Kiszallitas k2 = new Kiszallitas("Tigris", "Ugró bot", 5, "2017-12-24", true);
	...
	// static String[] honapok = { "Január", "Február", "Március", "Június",
	// "Július", "Augusztus", "Szeptember", "Október", "December" };
	...
	public void felulet() {
		...
		bal.add(mennyitText);
		bal.add(sor3);
		mennyit.setPreferredSize(new Dimension(100, 20));
		sor3.add(mennyit);

		// 4 sor
		bal.add(mikorText);
		bal.add(sor4);
		mikorEv.setPreferredSize(new Dimension(75, 20));
		mikorHo.setPreferredSize(new Dimension(50, 20));
		mikorNap.setPreferredSize(new Dimension(50, 20));
		sor4.add(mikorEv);
		sor4.add(mikorHo);
		sor4.add(mikorNap);
	...
	public void mentesDB() throws ClassNotFoundException, SQLException {
		...
		String resultMsg = db.ir(person, gift, pty, time, status);

		statusMsg.setText(resultMsg);
	}
	
	public void keresesDB() throws ClassNotFoundException, SQLException {
		Adatbazis db = new Adatbazis();
		String person = kinek.getText();
		String[] results = db.keres(person);
		
		if (results[0] != null) {
			kinek.setText(results[1]);
			mit.setText(results[2]);
			mennyit.setValue(Integer.parseInt(results[3]));
			String[] parts = results[4].split("-");
			mikorEv.setValue(Integer.parseInt(parts[0]));
			mikorHo.setValue(Integer.parseInt(parts[1]));
			mikorNap.setValue(Integer.parseInt(parts[2]));
			if (Integer.parseInt(results[5]) == 1) {
				option1.setSelected(true);
				option2.setSelected(false);
			} else {
				option1.setSelected(false);
	...
	public static void main(String[] args) {
		System.out.println("Adatbázisból olvassam az adatokat? (igen/nem)");
		Scanner sc = new Scanner(System.in);
		Felület uj = new Felület();
		
		
		if (sc.next().equals("igen")) {
			uj.felulet();
